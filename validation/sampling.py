...
def k_fold_cross_validation(k_count, init_model_func, save_path_func):
    ...
    for idx in range(0, k_count):
        ...
        train_data =  train_data + sampling_data
        print(len(train_data))
...
for k_count in [5, 10]:
    ...
    with open(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', f"trining_evaluate-sampling-{k_count}.csv"), 'w', newline="") as f:
        writer = csv.writer(f)
